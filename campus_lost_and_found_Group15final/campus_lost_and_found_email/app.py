...
def claim_item(item_id):
    item = Item.query.get_or_404(item_id)
    # Only allow claiming of found items by users who didn't post the item
    if item.type != 'found':
        flash('Only found items can be claimed.','danger'); return redirect(url_for('view_item', item_id=item.id))
    if current_user.id == item.user_id:
        flash('You cannot claim your own posting.','danger'); return redirect(url_for('view_item', item_id=item.id))

    proof = request.files.get('proof')
    idfile = request.files.get('idfile')
    if not proof or not idfile or proof.filename == '' or idfile.filename == '':
        flash('You must upload proof of ownership and an ID to submit a claim.','danger')
        return redirect(url_for('view_item', item_id=item.id))

    if not (allowed_file(proof.filename) and allowed_file(idfile.filename)):
        flash('Invalid file type for uploads.','danger'); return redirect(url_for('view_item', item_id=item.id))

    # save files
    pfname = secure_filename(proof.filename)
    idfname = secure_filename(idfile.filename)
    pfname = f"{int(os.times()[4])}_proof_{pfname}"
    idfname = f"{int(os.times()[4])}_id_{idfname}"
    proof.save(os.path.join(app.config['UPLOAD_FOLDER'], pfname))
    idfile.save(os.path.join(app.config['UPLOAD_FOLDER'], idfname))

    # compute similarity between proof and found item photo (if available)
    sim = None
    try:
        if item.photo:
            ph1 = imagehash.phash(Image.open(os.path.join(app.config['UPLOAD_FOLDER'], item.photo)))
            ph2 = imagehash.phash(Image.open(os.path.join(app.config['UPLOAD_FOLDER'], pfname)))
            dist = ph1 - ph2
            # number of bits in hash (e.g., 64)
            maxbits = ph1.hash.size
            sim = round(100 * (1 - (dist / maxbits)), 2)
    except Exception as e:
        sim = None

    item.status = 'pending_claim'
    item.claimed_by = current_user.id
    item.proof_photo = pfname
    item.id_photo = idfname
    item.similarity_score = sim
    # append to claim history
    hist = item.claim_history or ''
    import datetime
    entry = f"{datetime.datetime.utcnow().isoformat()} - claim submitted by user {current_user.id} - similarity={sim}\n"
    item.claim_history = hist + entry

    db.session.commit()
    flash('Claim submitted (pending). Proof and ID uploaded; admin will review.','info')
    return redirect(url_for('view_item', item_id=item.id))
